...
for %%F in ("%TEST_DIR%\*.docx") do (
    echo Uploading %%F...
    curl -X POST "http://"%HOST%"/api/upload-docx/" -F "file=@%%F" -o temp_response.json
...
for /f "tokens=1,2" %%i in (uploaded_files.txt) do (
    ...
    curl -X GET "http://%HOST%/api/download-docx/%%i" -o "%DOWNLOAD_DIR%\%%j"
)
...
for /f "tokens=1" %%i in (uploaded_files.txt) do (
    ...
    curl -X DELETE "http://%HOST%/api/delete-docx/%%i"
)
